...
namespace UsingADO.NET.Models
{
    public class BookDAL
    {
        ...
        {
            Book book = new Book();
            string qry = "select * from Book1 where id=@id";
            cmd = new SqlCommand(qry, con);
        ...
        {
            int result = 0;
            string qry = "insert into Book1 values(@name,@author,@price)";
            cmd = new SqlCommand(qry, con);
        ...
        {
            int result = 0;
            string qry = "update Book1 set name=@name,author=@author,price=@price where id=@id";
            cmd = new SqlCommand(qry, con);
            cmd.Parameters.AddWithValue("@name", book.Name);
            cmd.Parameters.AddWithValue("@author", book.Author);
            cmd.Parameters.AddWithValue("@price", book.Price);
        ...
        {
            int result = 0;
            string qry = "delete from Book1 where id=@id";
            cmd = new SqlCommand(qry, con);
            cmd.Parameters.AddWithValue("@id", id);
            con.Open();
            result = cmd.ExecuteNonQuery();
